...
if (!token) {
  ...
  console.error("Create a temporary Sanity API token with Editor permission and add SANITY_WRITE_TOKEN=... locally.");
  process.exit(1);
}
...
const assetsDir = path.resolve(here, "../content-seed/bayern/assets");
...
async function uploadOrReuse(filename) {
...
function img(assetId, alt, credit, key, extra = {}) {
  return {
    _type: "image",
    _key: key,
...
function ref(_ref, _key) {
  return { _type: "reference", _ref, _key };
}

function span(text, key = "s") {
...
function fact(key, label, value) {
  return { _type: "fact", _key: key, label, value };
}
...
const destination = {
  ...
  _type: "destination",
  title: "Reit im Winkl",
  slug: { _type: "slug", current: "reit-im-winkl" },
  ...
  places: [
    ref(IDS.course, "p-course"),
    ref(IDS.stay, "p-stay"),
    ref(IDS.eat, "p-eat"),
    ref(IDS.spa, "p-spa"),
  ],
  stories: [ref(IDS.story, "s-main")],
  featured: true,
...
const stay = {
  _id: IDS.stay,
  _type: "place",
  title: "Gut Steinbach Hotel & Chalets",
  slug: { _type: "slug", current: "gut-steinbach" },
  kicker: "STAY",
  ...
  placeType: "stay",
  destination: ref(IDS.destination, "dest"),
  ...
  goodToKnow: [
    fact("stay-f1", "Areal", "51 Hektar"),
    fact("stay-f2", "Chalets", "7 rund um den Naturweiher"),
    fact("stay-f3", "Best for", "Disconnecting · Families · Nature · Spa"),
  ],
  ...
  gallery: [
    img(uploaded.stayAerial.id, uploaded.stayAerial.alt, uploaded.stayAerial.credit, "stay-gallery-1"),
    img(uploaded.spa.id, uploaded.spa.alt, uploaded.spa.credit, "stay-gallery-2"),
  ],
  featured: true,
  aroundSelected: true,
  priority: 86,
...
const course = {
  _id: IDS.course,
  _type: "place",
  title: "Golfclub Reit im Winkl-Kössen",
  slug: { _type: "slug", current: "golfclub-reit-im-winkl-koessen" },
  kicker: "PLAY",
  ...
  goodToKnow: [
    fact("course-f1", "Löcher", "18"),
    fact("course-f2", "Grenzüberschreitend", "6 Löcher Tirol · 12 Löcher Bayern"),
    fact("course-f3", "Adresse", "Moserbergweg 60 · 6345 Kössen"),
  ],
  address: "Moserbergweg 60, 6345 Kössen, Österreich",
  website: "https://www.gcreit.de/",
  heroImage: img(uploaded.courseHero.id, uploaded.courseHero.alt, uploaded.courseHero.credit, "course-hero"),
  gallery: [
    img(uploaded.courseWide.id, uploaded.courseWide.alt, uploaded.courseWide.credit, "course-gallery-1"),
    img(uploaded.golfPerson.id, uploaded.golfPerson.alt, uploaded.golfPerson.credit, "course-gallery-2"),
    img(uploaded.border.id, uploaded.border.alt, uploaded.border.credit, "course-gallery-3"),
    img(uploaded.anton.id, uploaded.anton.alt, uploaded.anton.credit, "course-gallery-4"),
  ],
  featured: true,
  aroundSelected: true,
  priority: 85,
...
const eat = {
  ...
  slug: { _type: "slug", current: "restaurant-heimat-gut-steinbach" },
  kicker: "EAT",
  ...
  goodToKnow: [
    fact("eat-f1", "Prinzip", "80:80 – 80 % der Zutaten aus max. 80 km"),
    fact("eat-f2", "Don't miss", "Wiener Schnitzel"),
    fact("eat-f3", "Best for", "After 18 · Regional Food · Dinner"),
  ],
  website: "https://www.gutsteinbach.de/kulinarik/restaurant-heimat/",
  heroImage: img(uploaded.eat.id, uploaded.eat.alt, uploaded.eat.credit, "eat-hero"),
  gallery: [img(uploaded.schnitzel.id, uploaded.schnitzel.alt, uploaded.schnitzel.credit, "eat-gallery-1")],
  featured: false,
  aroundSelected: false,
  priority: 79,
...
const spa = {
  _id: IDS.spa,
  _type: "place",
  title: "Heimat & Natur SPA",
  slug: { _type: "slug", current: "heimat-und-natur-spa" },
  kicker: "DO",
  ...
  goodToKnow: [
    fact("spa-f1", "Fläche", "2.000 m²"),
    fact("spa-f2", "Pool", "16 Meter Indoorpool"),
    fact("spa-f3", "Auch möglich", "Day SPA mit Reservierung"),
  ],
  website: "https://www.gutsteinbach.de/wellness-spa/spa/",
  heroImage: img(uploaded.spa.id, uploaded.spa.alt, uploaded.spa.credit, "spa-hero"),
  featured: false,
  aroundSelected: false,
  priority: 77,
  seoTitle: "Heimat & Natur SPA | AROUND",
  seoDescription: "After Golf in Reit im Winkl: der Heimat & Natur SPA auf Gut Steinbach mit 2.000 m², Indoorpool und Day-Spa-Option.",
};
...
const story = {
  _id: IDS.story,
  _type: "story",
  title: "Das Heimatrefugium und die Zwei-Länder-Runde",
  slug: { _type: "slug", current: "heimatrefugium-zwei-laender-runde" },
  format: "worth-the-trip",
  kicker: "Bayern / Reit im Winkl",
  deck: "Ein Golfplatz zwischen Deutschland und Österreich. Ein Chalet mit Handy-Garage. Ein perfektes Schnitzel. Und die Erkenntnis, dass die beste Golfreise manchmal dort beginnt, wo Golf aufhört.",
  readingTime: 6,
  heroImage: img(uploaded.golfPerson.id, uploaded.golfPerson.alt, uploaded.golfPerson.credit, "story-hero"),
  body: storyBody,
  related: [
    ref(IDS.destination, "r-dest"),
    ref(IDS.stay, "r-stay"),
    ref(IDS.course, "r-course"),
    ref(IDS.eat, "r-eat"),
    ref(IDS.spa, "r-spa"),
  ],
  featured: true,
...
console.log("\n✓ AROUND Bayern content seed imported.");
console.log(`  Dataset: ${projectId}/${dataset}`);
console.log("  6 documents created/updated:");
for (const doc of docs) console.log(`  - ${doc._type}: ${doc.title}`);
console.log("\nOpen after deploy / refresh:");
console.log("  /destinations/reit-im-winkl");
console.log("  /places/gut-steinbach");
console.log("  /places/golfclub-reit-im-winkl-koessen");
console.log("  /places/restaurant-heimat-gut-steinbach");
console.log("  /places/heimat-und-natur-spa");
console.log("  /stories/heimatrefugium-zwei-laender-runde");
