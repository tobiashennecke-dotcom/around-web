...
export default function CampaignCalendar(){
 ...
 const overlaps=selectedEvent?events.filter(e=>e.id!==selectedEvent.id&&e.start<=selectedEvent.end&&e.end>=selectedEvent.start):[];
 const monthStart=month+"-01",monthEnd=year+"-"+pad(monthNumber)+"-"+pad(days);
 const monthEvents=visible.filter(e=>e.start<=monthEnd&&e.end>=monthStart);
 const pending=monthEvents.filter(e=>e.status==="requested").length;
