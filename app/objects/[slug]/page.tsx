...
export default async function ProductPage({params}:{params:Promise<{slug:string}>}){
  ...
  if(!product) notFound();
  return <main className="objectPage">
    <section className="hero objectHero" style={product.image?{backgroundImage:`linear-gradient(rgba(18,20,19,.2),rgba(18,20,19,.84)),url(${product.image})`}:undefined}>
    ...
    {product.whyItMatters&&<section className="section"><div className="container editorialGrid"><div><div className="eyebrow blue">WHY IT MATTERS</div><h2 className="sectionTitle">Nicht mehr.<br/>Besser.</h2></div><div className="featureCard objectTake"><p className="objectTakeQuote">{product.whyItMatters}</p></div></div></section>}
  </main>;
}
